...
//eigen1.Eigen doesn't seem to have a way to make a const tensor map out of a const float* ??
//So we have to cast away qualifiers to build it.
#pragma GCC diagnostic ignored "-Wcast-qual"

// eigen1.Eigen tensors are stored in column-major order, so an NHWC memory layout is given by Tensor<4>(C,W,H,N).
...
struct ComputeHandleInternal {
  ...
  //eigen1.Eigen::ThreadPool threadPool;
  //eigen1.Eigen::ThreadPoolDevice device;
...
struct ConvLayer {
  ...
  ConvLayer(const ConvLayerDesc& desc, int nnX, int nnY) {
    ...
    if(dilationX != 1 || dilationY != 1)
      throw StringError("eigen1.Eigen backend: Encountered convolution dilation factors other than 1, not supported");
      ...
      trunkTipActivation(desc.trunkTipActivation)
  {
    for (int i = 0; i < numBlocks; ++i) {
      if (desc.blocks[i].first == ORDINARY_BLOCK_KIND) {
        ...
        throw StringError("eigen1.Eigen backend: Dilated residual blocks are not supported right now");
      }
...
ComputeContext* NeuralNet::createComputeContext(
  ...
  if(useFP16)
    throw StringError("eigen1.Eigen backend: useFP16 = true not supported");
  if(!useNHWC)
    throw StringError("eigen1.Eigen backend: useNHWC = false not supported");
...
ComputeHandle* NeuralNet::createComputeHandle(
  ...
  if(logger != NULL) {
    logger->write("eigen1.Eigen (CPU) backend thread " + Global::intToString(serverThreadIdx) + ": Model version " + Global::intToString(loadedModel->modelDesc.version));
    logger->write("eigen1.Eigen (CPU) backend thread " + Global::intToString(serverThreadIdx) + ": Model name: " + loadedModel->modelDesc.name);
  }
  ...
  if(!inputsUseNHWC)
    throw StringError("eigen1.Eigen backend: inputsUseNHWC = false unsupported");
